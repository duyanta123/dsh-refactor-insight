...
- [ ] `npm run test:compat` 通过（DSH 0.1.5-rc.2 / Node 22.19+；临时 profile add、dump-config、启动 smoke test）
- [ ] `npm pack --dry-run` 通过
...
既有 Node 18/22 CI 仅代表独立诊断脚本回归；最新 DSH 0.1.5-rc.2 宿主要求 Node >=22.19。
